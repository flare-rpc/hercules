...
#include <flare/rapidjson/allocators.h>  // CrtAllocator (default) for Writer instantiation
#include <flare/rapidjson/encodings.h>  // UTF8 (default) for Writer instantiation
#include <flare/rapidjson/error/en.h>
...
namespace hercules::common {
    ...
    public:
        ...
        public:
            ...
            void Puts(const char *c, size_t size) { buffer_.append(c, size); }
        ...
        public:
            ...
            explicit Value(json_parser::Value &document, const ValueType type)
            ...
            TRITONJSON_STATUSTYPE Swap(json_parser::Value &other) {
            ...
            TRITONJSON_STATUSTYPE Add(const char *name, json_parser::Value &&value) {
                rapidjson::Value &object = AsMutableValue();
                if (!object.IsObject()) {
                    TRITONJSON_STATUSRETURN(
            ...
            TRITONJSON_STATUSTYPE Append(json_parser::Value &&value) {
            ...
            // error if not.
            TRITONJSON_STATUSTYPE AssertType(json_parser::ValueType type) const {
            ...
            TRITONJSON_STATUSTYPE At(
                    const size_t idx, json_parser::Value *value = nullptr) {
                rapidjson::Value &array = AsMutableValue();
                ...
                *value = json_parser::Value(array[idx], allocator_);
                return TRITONJSON_STATUSSUCCESS;
            }
            ...
            bool Find(const char *name, json_parser::Value *value) {
                rapidjson::Value &object = AsMutableValue();
                if (object.IsObject() && object.HasMember(name)) {
                    if (value != nullptr) {
                        *value = json_parser::Value(object[name], allocator_);
                    }
                    return true;
                }
            ...
            TRITONJSON_STATUSTYPE MemberAsArray(
                    const char *name, json_parser::Value *value) {
                rapidjson::Value &object = AsMutableValue();
                if (!object.IsObject() || !object.HasMember(name)) {
                    TRITONJSON_STATUSRETURN(
                ...
                *value = json_parser::Value(v, allocator_);
                return TRITONJSON_STATUSSUCCESS;
            }
            ...
            TRITONJSON_STATUSTYPE IndexAsArray(
                    const size_t idx, json_parser::Value *value) {
                rapidjson::Value &array = AsMutableValue();
                if (!array.IsArray() || (idx >= array.GetArray().Size())) {
                    TRITONJSON_STATUSRETURN(
                            std::string("attempt to access non-existing array index '") +
