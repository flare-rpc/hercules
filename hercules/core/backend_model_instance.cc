...
// Utilities for warmup feature
    ...
    TritonModelInstance::CreateInstances(
        ...
        static hercules::common::HostPolicyCmdlineConfig empty_host_policy;
        ...
        for (const auto &group : model_config.instance_group()) {
            ...
            for (int32_t c = 0; c < group.count(); ++c) {
                ...
                } else if (group.kind() == hercules::proto::ModelInstanceGroup::KIND_MODEL) {
                    instance_setting.emplace_back(
    ...
    TritonModelInstance::GenerateWarmupData() {
        ...
        for (const auto &warmup_setting : model_->Config().model_warmup()) {
            ...
            for (const auto &input_meta : warmup_setting.inputs()) {
                auto element_count =
                        hercules::common::GetElementCount(input_meta.second.dims());
                if (element_count == -1) {
                    return Status(
                            Status::Code::INVALID_ARG,
                ...
                int64_t batch_byte_size =
                        element_count *
                        hercules::common::GetDataTypeByteSize(input_meta.second.data_type());
                if (batch_byte_size == 0) {
                    batch_byte_size = element_count * sizeof(int32_t);
                }

                switch (input_meta.second.input_data_type_case()) {
                    case hercules::proto::ModelWarmup_Input::InputDataTypeCase::kZeroData:
                        max_zero_byte_size = std::max(batch_byte_size, max_zero_byte_size);
                        break;
                    case hercules::proto::ModelWarmup_Input::InputDataTypeCase::kRandomData: {
            ...
            for (size_t cnt = 0; cnt < warmup_setting.batch_size(); cnt++) {
                warmup_data.requests_.emplace_back(
                        new inference_request(model_, model_->Version()));
                auto &lrequest = warmup_data.requests_.back();

                // Second pass to prepare original inputs.
                std::vector<std::shared_ptr<inference_request::Input>> input_sps;
                for (const auto &input_meta : warmup_setting.inputs()) {
                    ...
                    switch (input_meta.second.input_data_type_case()) {
                        ...
                        case hercules::proto::ModelWarmup_Input::InputDataTypeCase::
                            kInputDataFile: {
                    ...
                    inference_request::Input *input = nullptr;
    ...
    TritonModelInstance::WarmUp() {
        ...
        std::vector<hercules::core::TritonModelInstance::WarmupData> lwarmup_samples;
    ...
    TritonModelInstance::Execute(
        ...
        TRITONBACKEND_ModelInstance *triton_model_instance =
                reinterpret_cast<TRITONBACKEND_ModelInstance *>(this);
        hercules_backend::hercules_model_instance_exec_func inst_exec_fn =
        ...
        if (err != nullptr) {
            ...
            for (TRITONBACKEND_Request *tr : triton_requests) {
                std::unique_ptr<inference_request> ur(
                        reinterpret_cast<inference_request *>(tr));
                inference_request::RespondIfError(ur, status, true /* release_requests */);
            }
