    ...
    std::unique_ptr<Scheduler>* scheduler)
{
  ...
  for (const hercules::proto::ModelSequenceBatching_State& state : states) {
  ...
  switch (initial_state.state_data_case()) {
    ...
    case hercules::proto::ModelSequenceBatching_InitialState::StateDataCase::
      ...
      RETURN_IF_ERROR(ReadTextFile(
          ...
          &file_input));
      if (initial_state.data_type() == hercules::proto::DataType::TYPE_STRING) {
...
GetBooleanOverrideInputs(
    ...
    const hercules::proto::DataType tensor_datatype, const float fp32_false_value,
    ...
    std::shared_ptr<inference_request::Input>* false_override)
{
  ...
  if (tensor_datatype == hercules::proto::DataType::TYPE_INT32) {
    *(reinterpret_cast<int32_t*>(true_p_ptr)) = int32_true_value;
    *(reinterpret_cast<int32_t*>(false_p_ptr)) = int32_false_value;
  } else if (tensor_datatype == hercules::proto::DataType::TYPE_FP32) {
...
bool
SequenceBatch::CreateCorrelationIDControl(const hercules::proto::ModelConfig& config)
{
  ...
  hercules::proto::DataType correlation_id_datatype;
  ...
  if (!correlation_id_tensor_name.empty()) {
    if ((correlation_id_datatype != hercules::proto::DataType::TYPE_UINT64) &&
        (correlation_id_datatype != hercules::proto::DataType::TYPE_INT64) &&
    ...
    const inference_request::SequenceId& corrid, const bool not_ready)
{
  ...
  if (seq_slot_corrid_override_ != nullptr) {
    ...
    if (seq_corr_id->DType() == hercules::proto::DataType::TYPE_STRING) {
      ...
      // length in bytes
      size_p = 4 + hercules::core::STRING_CORRELATION_ID_MAX_LENGTH_BYTES;
    }
