                ...
                model_action_fn_ = [](TRITONREPOAGENT_Agent *agent,
                    ...
                    switch (action_type) {
                        case TRITONREPOAGENT_ACTION_LOAD: {
                            ...
                            const auto &files =
                                    *reinterpret_cast<std::vector<const inference_parameter *> *>(
                                            agent_model->State());
...
// Helper class to manage the lifecycle of a list of associated agent models
        ...
        private:
            FLARE_DISALLOW_COPY_AND_ASSIGN(TritonRepoAgentModelList);
        ...
        CreateAgentModelListWithLoadAction(
                const hercules::proto::ModelConfig &original_model_config,
            ...
            if (original_model_config.has_model_repository_agents()) {
                ...
                hercules::proto::ModelConfig model_config = original_model_config;
        ...
        VersionsToLoad(
                ...
                const hercules::proto::ModelConfig &model_config, std::set<int64_t> *versions) {
            versions->clear();
...
// to UNAVAILABLE if all smart pointer copies are out of scope
    ...
    struct ModelRepositoryManager::ModelInfo {
        ...
        hercules::proto::ModelConfig model_config_;
    ...
    public:
        static Status Create(
                inference_server *server, const double min_compute_capability,
    ...
    private:
        ...
        ModelLifeCycle(
                const double min_compute_capability, inference_server *server,
                const hercules::common::BackendCmdlineConfigMap &backend_cmdline_config_map,
                const hercules::common::HostPolicyCmdlineConfigMap &host_policy_map,
                const unsigned int model_load_thread_count)
            ...
            load_pool_.reset(
                    new hercules::common::ThreadPool(std::max(1u, model_load_thread_count)));
        }
        ...
        const hercules::common::BackendCmdlineConfigMap cmdline_config_map_;
        const hercules::common::HostPolicyCmdlineConfigMap host_policy_map_;

        // Fixed-size thread pool to load models at specified concurrency
        std::unique_ptr<hercules::common::ThreadPool> load_pool_;
    };
    ...
    ModelRepositoryManager::Create(
        ...
        if (!model_control_enabled || load_all_models_on_startup) {
            ...
            std::unordered_map<std::string, std::vector<const inference_parameter *>>
                    models;
    ...
    ModelRepositoryManager::Poll(
        ...
        *all_models_polled = true;
        ...
        for (const auto &pair : model_to_path) {
            ...
            static std::vector<const inference_parameter *> empty_params;
    ...
    ModelRepositoryManager::InitializeModelInfo(
        ...
        if (linfo->model_path_.empty()) {
            ...
            RETURN_IF_ERROR(TritonRepoAgentModel::Create(
                    TRITONREPOAGENT_ARTIFACT_FILESYSTEM, "", hercules::proto::ModelConfig(),
